...
const AdminManagerAttendance = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      setLoading(true);
      const attendanceRef = collection(db, 'm_attendance');
      ...
      querySnapshot.forEach((userDoc) => {
        ...
        // Check if this user has the current admin as their supervisor
        if (userData.supervisorUid === currentAdminUid) {
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.text(`Manager Attendance Report - ${selectedDate}`, 14, 15);
    ...
    doc.save(`manager_attendance_${selectedDate.replace(/\//g, '-')}.pdf`);
  };
  ...
  return (
    <>
      ...
      <div className="daily-attendance-container">
        ...
        <SearchBar placeholder="Search by manager name..." value={searchQuery} onChange={setSearchQuery} />
