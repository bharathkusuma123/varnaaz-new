      ...
      querySnapshot.forEach((doc) => {
        ...
        adminLeaves.forEach(leave => {
          allLeaves.push({
            ...
            adminUid: doc.id,
            docId: doc.id,
            leaveIndex: adminLeaves.indexOf(leave)
          });
        });
      });
                  ...
                  {paginatedLeaves.map((leave, index) => (
                    <tr key={`${leave.adminUid}_${leave.leaveId}`}>
                      ...
                      <td>
                        {leave.status === 'Pending' ? (
                          <select 
                            className={`form-select form-select-sm ${leave.status === 'Approved' ? 'bg-success text-white' : 
                                      leave.status === 'Rejected' ? 'bg-danger text-white' : 'bg-warning text-dark'}`}
                            value={leave.status}
