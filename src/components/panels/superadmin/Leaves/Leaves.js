...
const SuperAdminLeavesPage = () => {
  const [user, setUser] = useState({ name: '', uid: 'shashank123' });
  ...
  const fetchUserName = async () => {
    try {
      const userDoc = await getDoc(doc(db, 'sa_employeeLeaves', user.uid));
      if (userDoc.exists()) {
        setUser((prev) => ({ ...prev, name: userDoc.data().employeeName || 'Employee' }));
      }
    } catch (err) {
  ...
  const fetchLeaves = async () => {
    setLoading(true);
    try {
      const docRef = doc(db, 'sa_employeeLeaves', user.uid);
...
export default SuperAdminLeavesPage;
