...
const SuperAdminNavbar = () => {
  ...
  const handleLogout = () => {
    // You can add logout logic here if needed (e.g., clear auth state)
    setIsMobile(false);
    navigate('/');
  };
    ...
    <nav className="navbar">
      <div className="navbar-container">
        ...
        <div className={`nav-menu ${isMobile ? 'active' : ''}`}>
          ...
          <NavLink to="/sa-taskmanager" className="nav-item" onClick={() => setIsMobile(false)}>
            Task Manager
          </NavLink>

          {/* Logout Icon (visible on mobile too) */}
